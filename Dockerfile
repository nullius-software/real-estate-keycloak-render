...
EXPOSE 8080
ENTRYPOINT ["/opt/keycloak/bin/kc.sh"]
CMD ["start", "--optimized", "--host=0.0.0.0", "--https-port=8080", "--hostname=real-estate-keycloak-render.onrender.com", "--https-certificate-file=/opt/keycloak/conf/server.crt", "--https-certificate-key-file=/opt/keycloak/conf/server.key", "--proxy-headers=forwarded", "--cache=local", "-Dquarkus.datasource.jdbc.acquisition-timeout=60s"]
